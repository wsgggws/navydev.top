...
```text
src/
  App.tsx                  # React 壳：挂载 stage，处理自动序幕、控制条和 MovieDirector
...
| 顺序 | ID         | 标题           | 叙事角色                               |
| ---- | ---------- | -------------- | -------------------------------------- |
| 1    | `scene-01` | 关于我         | 身份、原则与兴趣构成动态人物肖像       |
| 2    | `scene-02` | 从问题出发     | 问题、证据和最小改动组成判断路径       |
| 3    | `scene-03` | 让系统彼此听懂 | 需求经过契约、追踪和恢复完整返回       |
| 4    | `scene-04` | 工具退到身后   | 思考、构建、运行与交付组成工作流       |
| 5    | `scene-05` | 屏幕之外       | 阅读、跑步和观察组成生活留白           |
| 6    | `scene-06` | 继续提问       | 为什么、如果与下一步把好奇带向行动     |
| 7    | `scene-07` | 继续创作       | 创作循环结束后进入独立片尾“未完待续”   |
